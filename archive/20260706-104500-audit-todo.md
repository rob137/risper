...
- [x] **Commit the live-but-uncommitted Wayland fix.** (done 2026-07-06 10:41)
  The legacy Linux platform module had been dirty since 2026-06-16 (Wayland-first
  `session_type` default for the systemd environment) — and because the wrappers
  run straight out of the repository, that diff WAS the running daemon's
...
- [x] **Delete dead modules or say why they stay.** (done 2026-07-06 10:44)
  Deleted: the monitor, overlay, and audio-level modules and their two test files,
  and the `show_overlay` config knob the daemon explicitly ignored.
  The status window (`risper-status`) was the one status UI. Rationale in
