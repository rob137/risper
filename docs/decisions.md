...
- `desktop.Play` remains a reaped, synchronous primitive for lifecycle-safe callers, while `desktop.PlayAsync` lets the transcription-start sound overlap the stop-to-paste pipeline. The toggle waits on that handle only at process exit, keeping temporary-directory cleanup deterministic without making Rob wait through the 2.18s service-login sample before transcription starts. Recording-start and success sounds still only delay process exit.
