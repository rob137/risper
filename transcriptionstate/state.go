// Package transcriptionstate owns the durable marker for an in-flight
...
func Cancel(cfg config.Config, state *State) error {
	...
	if metadata != nil {
		if err := appendLog(filepath.Join(state.SessionDir, session.StatusLogFile), message); err != nil {
			return err
		}
		if _, err := events.Append(state.SessionDir, "transcription.cancel_requested", map[string]any{
